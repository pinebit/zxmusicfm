...
The track list is a single scrollable collection. Search, filtering, and categories are outside the MVP. It also offers **Auto-Play Next** and **Shuffle** as independent toggles. On a user's first visit, Auto-Play Next is enabled and Shuffle is disabled. Their settings persist across page reloads. Shuffle avoids repeating a track until every track in the current collection has played. When Auto-Play Next reaches the end of the collection, playback wraps to the beginning and continues.
...
The active track shows elapsed and total time. Inactive rows show total duration only. Times use a compact `m:ss` representation for tracks under one hour and `h:mm:ss` when needed.
...
The UI presents those three envelopes as one overlaid waveform and one seek control. Channel A is always red, B is always yellow, and C is always cyan; these colors identify logical channels and do not change when `channelLayout` changes their stereo positions. Translucent fills or strokes allow overlapping activity to remain visible through alpha blending. All three channels are always visible in the MVP, with no per-channel waveform toggles. Playback progress is applied consistently across all three layers, making the played and unplayed regions distinguishable by more than color alone. The accessible name, current value, keyboard behavior, and pointer/touch hit target belong to the combined seek control rather than to three separate interactive elements.
